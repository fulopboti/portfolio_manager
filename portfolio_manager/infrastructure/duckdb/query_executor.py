...
from portfolio_manager.infrastructure.data_access.query_executor import (
    QueryExecutor,
    QueryResult,
)
...
class DuckDBQueryExecutor(QueryExecutor):
    ...
    async def fetch_all(
        ...
        # Handle non-dict rows (lists, tuples, or single values)
        return [tuple(row) if isinstance(row, list | tuple) else (row,) for row in result.rows]  # type: ignore[unreachable]

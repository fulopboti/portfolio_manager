...
class YFinanceProvider(DataProvider):
    ...
    async def get_ohlcv_data(
        ...
        try:
            ...
            if isinstance(e, DataIngestionError):
                ...
                raise DataIngestionError(f"Failed to fetch data for {symbol}: {str(e)}")
    ...
    async def get_fundamental_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self._failed_requests += 1
            raise DataIngestionError(f"Failed to fetch fundamental data for {symbol}: {str(e)}")
    ...
    def _fetch_ticker_data(self, symbol: str, start_date: datetime, end_date: datetime) -> Optional[pd.DataFrame]:
    ...
    def _fetch_fundamental_data(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            fundamental_data['last_updated'] = datetime.now(timezone.utc).isoformat()
    ...
    def _create_asset_snapshot(self, symbol: str, date: pd.Timestamp, row: pd.Series) -> Optional[AssetSnapshot]:

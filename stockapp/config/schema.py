...
class DatabasePoolConfig(BaseModel):
    ...
    max_connections: int = Field(10, ge=1, le=100, description="Maximum number of connections")
...
class EventBusConfig(BaseModel):
    ...
    max_concurrent_events: int = Field(100, ge=1, description="Maximum concurrent events")
...
class RiskManagementConfig(BaseModel):
    ...
    max_position_size: float = Field(0.10, gt=0, le=1, description="Maximum position size as fraction")
    max_sector_exposure: float = Field(0.25, gt=0, le=1, description="Maximum sector exposure")
    stop_loss_threshold: float = Field(-0.05, ge=-1, le=0, description="Stop loss threshold")
...
class ApplicationConfig(BaseModel):
    ...
    name: str = Field("StockApp", description="Application name")
...
class StockAppConfig(BaseModel):
    """Complete StockApp configuration schema."""
...
def validate_config(config_dict: dict) -> StockAppConfig:
    """
    ...
    """
    return StockAppConfig(**config_dict)